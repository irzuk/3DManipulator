#include<Servo.h>
#define TRIG_PIN 2            //Connect TRIG pin to digital pin 4
#define ECHO_PIN 4
...
int servoPort[5] = {6,7,8,9,10};

int minDegree[5] = {0,80,100,9,10};
int maxDegree[5] = {180,170,180,9,10};
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  float angle_z = gyro_angle_z;  //Accelerometer doesn't give z-angle
   ...
   int xx = round(angle_x);
  Serial.print(xx);
  Serial.print(',');
  
  int yy = round(angle_y);
  Serial.print(yy);
  Serial.print(',');
}

void moveServo(int i, int degree){
   if(degree < maxDegree[i] && degree > minDegree[i]) {
      //Serial.print(degree);
      //Serial.print('!');
      servo[i].write(degree);
   } 
  }


//source https://create.arduino.cc/projecthub/ioarvanit/measuring-distance-with-sound-353c17
 int countDistance() {
